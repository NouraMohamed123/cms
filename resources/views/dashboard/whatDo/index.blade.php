...
<div class="d-flex align-items-center justify-content-between mb-3">
    <ol class="breadcrumb breadcrumb-dot text-muted fs-6 fw-semibold">
        ...
        <li class="breadcrumb-item pe-3 text-primary">{{ trans('advantage.feature') }} </li>
    </ol>
    <div class="d-flex">
                ...
                <div class="card-body fs-6 p-5 text-gray-700">
                    <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_teachers">
                        ...
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                            <th>#</th>
                            <th class="min-w-100px"> {{ trans('advantage.name') }}  </th>
                          
                            <th class="min-w-100px"> {{ trans('advantage.desc') }}   </th>
                         
                            <th class="text-end min-w-100px pe-5">{{ trans('advantage.setting') }} </th>
                        </tr>
                        <!--end::Table row-->
                        </thead>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-header">
            <h1 class="modal-title fs-5" id="exampleModalLabel">   {{ trans('advantage.Add_feature') }}  </h1>
        ...
        <div class="modal-body">
            <form action="{{ route('dashboard.advantages.store')  }}" method="post" class="row"  enctype="multipart/form-data">
                ...
                @method('post')
                <div class="col-md-6">
                    <input type="text" class="form-control mb-2 question" placeholder="{{ trans('advantage.name_ar') }} " name="name" required>
                </div>
                <div class="col-md-6">
                    <input type="text" class="form-control mb-2 question_en" placeholder=" {{ trans('advantage.name_en') }}" name="name_en" required>
                </div>
                <div class="col-md-6">
                    <textarea id="" class="form-control mb-2 answer" placeholder=" {{ trans('advantage.desc_ar') }}" name="desc" required></textarea>
                </div>
                <div class="col-md-6">
                    <textarea  id="" class="form-control mb-2 answer_en" placeholder=" {{ trans('advantage.desc_en') }}" name="desc_en" required></textarea>
                </div>
                <div class="col-md-6">
                <input type="file" name="avatar" accept=".png, .jpg, .jpeg" />
               </div>
                <button type="submit" class="btn btn-primary save">{{ trans('advantage.add') }}</button>
            </form>
        </div>
        </div>
    </div>
