...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <ol class="breadcrumb breadcrumb-dot text-muted fs-6 fw-semibold">
            ...
            <li class="breadcrumb-item pe-3 text-primary">كل الصلاحيات</li>
        </ol>
        @if(auth()->user()->hasPermission('roles_create'))
            <a href="{{  route('dashboard.roles.create') }}" class="btn btn-primary">اضافة  </a>
        @endif
    </div>
        ...
        <div class="card-body fs-6 p-5 text-gray-700">
            <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_teachers">
                ...
                <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                    ...
                    <th class="min-w-100px"> الصلاحيات </th>
                    <th class="min-w-100px">عدد المستخدمين</th>
                ...
                @foreach($roles as $index=> $role)
                    ...
                    <td>
                        ...
                        <td> <span class="badge badge-circle badge-secondary"> {{ $role->users_count  }} </span></td>
                    <td class="text-end">
                        <div class="btn-actions d-flex align-items-center justify-content-end">
                            ...
                            @if(auth()->user()->hasPermission('roles_delete'))
                                <form action="{{ route('dashboard.roles.destroy', $role->id)  }}" method="post" class="d-inline-block">
                                    @csrf
                                    @method('delete')
